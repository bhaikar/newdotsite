...
const SocialChannelCard = ({ href, label, handle, meta, icon, iconBg, accent }: SocialChannelCardProps) => {
  return (
    <a
      href={href}
      target="_blank"
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate font-mono text-xs text-slate-400">{handle}</span>
      </span>

      <span className="hidden shrink-0 font-mono text-[11px] uppercase tracking-wider text-slate-500 transition-colors duration-300 group-hover:text-[var(--card-accent)] sm:block">
        {meta}
      </span>
